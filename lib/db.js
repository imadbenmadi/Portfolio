...
async function ensureCertificatesSchema() {
  ...
  `
  await sql`ALTER TABLE certificates ADD COLUMN IF NOT EXISTS sort_order INTEGER DEFAULT 0`
  await sql`ALTER TABLE certificates ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW()`
  await sql`ALTER TABLE certificates ADD COLUMN IF NOT EXISTS file_type VARCHAR(50) DEFAULT 'application/pdf'`
}
...
export async function createCertificate(data) {
  await ensureCertificatesSchema()
  const { rows } = await sql`
    INSERT INTO certificates (title, issuer, issue_date, file_url, file_type, sort_order)
    VALUES (
      ${data.title},
      ${data.issuer || null},
      ...
      (SELECT COALESCE(MAX(sort_order), -1) + 1 FROM certificates)
    )
    RETURNING *
  `
  return rows[0] || null
